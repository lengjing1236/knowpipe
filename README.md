...
面向"大数据综合实践"课程的仓库，包含两部分，彼此独立运行、互不调用：

- **课程项目**（`knowpipe/mining/`、`knowpipe/web/`）：基于 Spark 与 MongoDB 的多源
  技术内容知识挖掘与个性化发现 Web 系统，对应
  ...
  **这是本仓库当前的主要交付物**。
- **个人工具**（`knowpipe/` 根目录下的其余模块）：一个独立维护的知识精炼管道 CLI
  （见 [DESIGN.md](DESIGN.md)），课程项目不依赖它，二者只是共享同一个 `knowpipe`
  包名。

## 目录

- [课程项目：Spark + MongoDB 知识挖掘与个性化 Web](#课程项目spark--mongodb-知识挖掘与个性化-web)
  - [架构](#架构)
  - [环境准备](#环境准备)
  ...
  - [测试](#测试)
  - [课程过程文档](#课程过程文档)
- [个人工具：知识精炼管道 CLI](#个人工具知识精炼管道-cli)

---

## 课程项目：Spark + MongoDB 知识挖掘与个性化 Web
