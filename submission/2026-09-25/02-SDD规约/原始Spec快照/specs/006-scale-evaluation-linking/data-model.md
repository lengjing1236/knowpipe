...
[方案] Linking run：run_id唯一，algorithm_version、corpus_sha256、参数、Spark信息、输入数、status；全量结果保存完才更新linking_state._id=current指针。Links：(run_id,episode_id,segment_id,source,doc_id)唯一；节目batch_id通过run.episodes关联；文献/节目内容指纹、score、shared_terms。读取过滤缺失/变化文献和变化节目。无共同词结果为empty；未发布为pending；节目过期为stale。
